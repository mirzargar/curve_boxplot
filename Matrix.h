...
// *********
// Simple Matrix data structure to represent scalar fields
// *********
...
// This class provide a basic Matrix object inorder to handle fields of values easiers.
template <class T>
class Matrix{
...
Matrix<T> Matrix<T>::SubRows(const vector<int>& r){
    ...
    for(std::vector<int>::size_type i=0; i !=r.size(); ++i)
        for(unsigned int j=0;j<m.Ydim;++j){
            if(r[i]>= Xdim){
                printf("Matrix: out of bound index: %d\n", r[i]);
                exit(-1);
            }
            m.p[i][j] = p[r[i]][j];
        }
...
void Matrix<T>::loadMatrixFile(unsigned int xdim, unsigned int ydim, string fileName){
    //TODO: handle filename invalid case
